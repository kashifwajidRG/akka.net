// //-----------------------------------------------------------------------
// // <copyright file="RGRemoteActorRefProvider.cs" company="Akka.NET Project">
...
{
    public class RGRemoteActorRefProvider : IRemoteActorRefProvider
    {
        private readonly ILoggingAdapter _log;
        private readonly RGLocalActorRefProvider _local;
        private volatile RGInternals _internals;
        ...
        public RGRemoteActorRefProvider(string systemName, Settings settings, EventStream eventStream)
        {
            settings.InjectTopLevelFallback(RGRemoteConfigFactory.Default());

            var remoteDeployer = new RGRemoteDeployer(settings);
            IInternalActorRef DeadLettersFactory(ActorPath path) => new RGRemoteDeadLetterActorRef(this, path, eventStream);
            _local = new RGLocalActorRefProvider(systemName, settings, eventStream, remoteDeployer, DeadLettersFactory);
            RemoteSettings = new RemoteSettings(settings.Config);
            Deployer = remoteDeployer;
            _log = _local.Log;
        }
        
        private RGInternals RemoteInternals
        {
            get { return _internals; }
        }

        private RGInternals CreateInternals()
        {
            var internals =
                new RGInternals(new RGRemoting(_system, this), _system.Serialization,
                    new RGRemoteSystemDaemon(_system, RootPath / "remote", RootGuardian, _remotingTerminator, _log));
        ...
        private volatile RGActorRefResolveThreadLocalCache _actorRefResolveThreadLocalCache;
        private volatile RGActorPathThreadLocalCache _actorPathThreadLocalCache;
        ...
        public virtual void Init(ActorSystemImpl system)
        {
            ...
            _actorRefResolveThreadLocalCache = RGActorRefResolveThreadLocalCache.For(system);
            _actorPathThreadLocalCache = RGActorPathThreadLocalCache.For(system);

            _remotingTerminator =
                _system.SystemActorOf(
                    RemoteSettings.ConfigureDispatcher(Props.Create(() => new RGRemotingTerminator(_local.SystemGuardian))),
                    "rg-remoting-terminator");
            ...
            if (propsDeploy.Scope is RemoteScope)
            {
                ...
                try
                {
                    ...
                    var rpath = (new RootActorPath(addr) / "remote" / localAddress.Protocol / localAddress.HostPort() /
                        ...
                        WithUid(path.Uid);
                    var remoteRef = new RGRemoteActorRef(Transport, localAddress, rpath, supervisor, props, deployment);
                    return remoteRef;
                }
                catch (Exception ex)
                {
        ...
        /// </summary>
        class RGInternals : INoSerializationVerificationNeeded
        {
            /// <summary>
            /// TBD
            /// </summary>
        ...
        /// </summary>
        private class RGRemotingTerminator : FSM<TerminatorState, RGInternals>, IRequiresMessageQueue<IUnboundedMessageQueueSemantics>
        {
            private readonly IActorRef _systemGuardian;
            private readonly ILoggingAdapter _log;

            public RGRemotingTerminator(IActorRef systemGuardian)
            {
            ...
            private void InitFSM()
            {
                ...
                {
                    var internals = @event.FsmEvent as RGInternals;
                    if (internals != null)
                    {
        ...
        private class RGRemoteDeadLetterActorRef : DeadLetterActorRef
        {
            public RGRemoteDeadLetterActorRef(IActorRefProvider provider, ActorPath actorPath, EventStream eventStream)
