// //-----------------------------------------------------------------------
// // <copyright file="RGRemoteSystemDaemon.cs" company="Akka.NET Project">
...
namespace RGLoggingAndTracing.RGRemote
{
    ...
    public interface RGIDaemonMsg { }

    /// <summary>
    ///  INTERNAL API
    /// </summary>
    public class RGDaemonMsgCreate : RGIDaemonMsg
    {
        public RGDaemonMsgCreate(Props props, Deploy deploy, string path, IActorRef supervisor)
        {
            Props = props;
            Deploy = deploy;
            Path = path;
    ...
    public class RGRemoteSystemDaemon : RGVirtualPathContainer
    {
        ...
        public RGRemoteSystemDaemon(ActorSystemImpl system, ActorPath path, IInternalActorRef parent,IActorRef terminator, ILoggingAdapter log)
            : base(system.Provider, path, parent, log)
        {
            _terminator = terminator;
            _system = system;
            RGAddressTerminatedTopic.Get(system).Subscribe(this);
        }
        ...
        protected override void TellInternal(object message, IActorRef sender)
        {
            ...
            if (message is RGIDaemonMsg)
            {
                Log.Debug("Received command [{0}] to RGRemoteSystemDaemon on [{1}]", message, Path.Address);
                if (message is RGDaemonMsgCreate) HandleDaemonMsgCreate((RGDaemonMsgCreate)message);
            }
            ...
            else if (message is RGAddressTerminated)
            {
                var addressTerminated = (RGAddressTerminated)message;
        ...
        /// </summary>
        /// <param name="message">The message.</param>
        private void HandleDaemonMsgCreate(RGDaemonMsgCreate message)
        {
            ...
            if(ActorPath.TryParse(message.Path, out childPath))
            {
                ...
                if (isTerminating)
                {
                    Log.Error("Skipping [{0}] to RGRemoteSystemDaemon on [{1}] while terminating", message, p.Address);
                }
        ...
        public override IActorRef GetChild(IEnumerable<string> name)
        {
            ...
            while (true)
            {
                var nameAndUid = RGActorCell.SplitNameAndUid(path);
                if (TryGetChild(nameAndUid.Name, out var child))
                {
                    if (nameAndUid.Uid != RGActorCell.UndefinedUid && nameAndUid.Uid != child.Path.Uid)
