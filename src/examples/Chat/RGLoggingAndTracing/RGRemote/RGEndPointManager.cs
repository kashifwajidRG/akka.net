// //-----------------------------------------------------------------------
// // <copyright file="RGEndPointManager.cs" company="Akka.NET Project">
...
{
    public class RGEndpointManager : ReceiveActor, IRequiresMessageQueue<IUnboundedMessageQueueSemantics>
    {
        ...
        /// </summary>
        public abstract class RGEndpointPolicy
        {
            /// <summary>
            ...
            protected RGEndpointPolicy(bool isTombstone)
            {
        ...
        /// </summary>
        public sealed class RGPass : RGEndpointPolicy
        {
            /// <summary>
            /// TBD
            /// </summary>
            ...
            public RGPass(IActorRef endpoint, int? uid)
                : base(false)
            {
                Uid = uid;
                Endpoint = endpoint;
        ...
        /// </summary>
        public sealed class RGGated : RGEndpointPolicy
        {
            public RGGated(Deadline deadline)
                : base(true)
            {
        ...
        /// </summary>
        public sealed class RGQuarantined : RGEndpointPolicy
        {
            public RGQuarantined(int uid, Deadline deadline)
                : base(true)
            {
                Uid = uid;
                Deadline = deadline;
        ...
        /// </summary>
        public abstract class RGRemotingCommand : INoSerializationVerificationNeeded { }

        /// <summary>
        /// TBD
        /// </summary>
        public sealed class RGListen : RGRemotingCommand
        {
            /// <summary>
            /// TBD
            /// </summary>
            /// <param name="addressesPromise">TBD</param>
            public RGListen(TaskCompletionSource<IList<ProtocolTransportAddressPair>> addressesPromise)
            {
        ...
        /// TBD
        /// </summary>
        public sealed class StartupFinished : RGRemotingCommand { }

        /// <summary>
        /// TBD
        /// </summary>
        public sealed class ShutdownAndFlush : RGRemotingCommand { }

        /// <summary>
        /// TBD
        /// </summary>
        public sealed class RGSend : RGRemotingCommand, IHasSequenceNumber
        {
            /// <summary>
            /// TBD
            /// </summary>
            ...
            public RGSend(object message, RemoteActorRef recipient, IActorRef senderOption = null, SeqNo seqOpt = null)
            {
                Recipient = recipient;
            ...
            /// <returns>TBD</returns>
            public RGSend Copy(SeqNo opt)
            {
                return new RGSend(Message, Recipient, SenderOption, opt);
            }
        }
        ...
        /// TBD
        /// </summary>
        public sealed class RGQuarantine : RGRemotingCommand
        {
            /// <summary>
            /// TBD
            /// </summary>
            ...
            /// <param name="uid">TBD</param>
            public RGQuarantine(Address remoteAddress, int? uid)
            {
                Uid = uid;
        ...
        /// TBD
        /// </summary>
        public sealed class ManagementCommand : RGRemotingCommand
        {
            /// <summary>
            /// TBD
            /// </summary>
        ...
        public sealed class ResendState : IEquatable<ResendState>
        {
            ...
            public ResendState(int uid, AckedReceiveBuffer<Tcp.Message> buffer)
            {
                Buffer = buffer;
                Uid = uid;
            }
            ...
            /// TBD
            /// </summary>
            public AckedReceiveBuffer<Tcp.Message> Buffer { get; private set; }
        ...
        private void KeepQuarantinedOr(Address remoteAddress, Action body)
        {
            ...
            if (uid.HasValue)
            {
                _log.Info(
                    ...
                    remoteAddress);
                // Restoring Quarantine marker overwritten by a RGPass(endpoint, refuseUid) pair while probing remote system.
        ...
        protected void Accepting()
        {
            ...
            Receive<Quarantine>(quarantine =>
            {
                ...
                if (policy.Item1 is RGPass pass && policy.Item2 == null)
                {
                ...
                else if (policy.Item1 is RGPass p && policy.Item2 != null)
                {
                    var uidOption = p.Uid;
        ...
        private void HandleInboundAssociation(InboundAssociation ia, bool writerIsIdle)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    switch (policy)
                    {
                        case RGPass pass when !pass.Uid.HasValue: // pass, but UID is unknown
                            ...
                            break;
                        case RGPass pWithUid: // pass with known UID
