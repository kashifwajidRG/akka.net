// //-----------------------------------------------------------------------
// // <copyright file="RGLruBoundedCache.cs" company="Akka.NET Project">
...
{
    public static class RGFastHash
    {
        /// <summary>
    ...
    public sealed class RGCacheStatistics
    {
        public RGCacheStatistics(int entries, int maxProbeDistance, double averageProbeDistance)
        {
            Entries = entries;
    ...
    public abstract class RGLruBoundedCache<TKey, TValue> where TValue : class
    {
        protected RGLruBoundedCache(int capacity, int evictAgeThreshold)
        {
            if (capacity <= 0)
        ...
        public RGCacheStatistics Stats
        {
            get
            {
                var i = 0;
                ...
                return new RGCacheStatistics(count, max, (double)sum / count);
            }
        }
        ...
        {
            return $"RGLruBoundedCache(values = [{string.Join<TValue>(",", _values)}], hashes = [{string.Join(",", _hashes)}], " +
