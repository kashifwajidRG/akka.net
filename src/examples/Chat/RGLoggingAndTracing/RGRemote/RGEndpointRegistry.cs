// //-----------------------------------------------------------------------
// // <copyright file="RGEndPointRegistry.cs" company="Akka.NET Project">
...
{
    public class RGEndpointRegistry
    {
        ...
        private Dictionary<Address, RGEndpointManager.RGEndpointPolicy> _addressToWritable =
            new Dictionary<Address, RGEndpointManager.RGEndpointPolicy>();
        ...
        public IActorRef RegisterWritableEndpoint(Address address, IActorRef endpoint, int? uid)
        {
            ...
            {
                if (existing is RGEndpointManager.RGPass pass) // if we already have a writable endpoint....
                {
            ...
            _addressToWritable[address] = new RGEndpointManager.RGPass(endpoint, uid);
            _writableToAddress[endpoint] = address;
            return endpoint;
        }

        /// <summary>
        /// Sets the UID for an existing <see cref="RGEndpointManager.RGPass"/> policy.
        /// </summary>
        ...
        public void RegisterWritableEndpointUid(Address remoteAddress, int uid)
        {
            ...
            {
                if (existing is RGEndpointManager.RGPass pass)
                    _addressToWritable[remoteAddress] = new RGEndpointManager.RGPass(pass.Endpoint, uid);
                // if the policy is not RGPass, then the GotUid might have lost the race with some failure
            }
        }
        ...
        public bool IsQuarantined(Address address, int uid)
        {
            ...
            var policy = WritableEndpointWithPolicyFor(address) as RGEndpointManager.RGQuarantined;
            switch (policy)
            {
                case RGEndpointManager.RGQuarantined q when q.Uid == uid:
                    return true;
                default:
        ...
        public int? RefuseUid(Address address)
        {
            ...
            switch (policy)
            {
                case RGEndpointManager.RGQuarantined q:
                    return q.Uid;
                default:
                    if (_addressToRefuseUid.ContainsKey(address))
        ...
        /// </summary>
        /// <param name="address">The remote address to check.</param>
        /// <returns>The <c>RGEndpointManager.EndpointPolicy</c> if we have one for this address, <c>null</c> otherwise.</returns>
        public RGEndpointManager.RGEndpointPolicy WritableEndpointWithPolicyFor(Address address)
        {
        ...
        {
            var policy = WritableEndpointWithPolicyFor(address);
            return policy is RGEndpointManager.RGPass;
        }
        ...
        public void MarkAsFailed(IActorRef endpoint, Deadline timeOfRelease)
        {
            if (IsWritable(endpoint))
            {
                ...
                {
                    if (policy is RGEndpointManager.RGQuarantined)
                    {
                        // don't overwrite RGQuarantined with Gated
                    }
                    else if (policy is RGEndpointManager.RGPass)
                    {
                        _addressToWritable[address] = new RGEndpointManager.RGGated(timeOfRelease);
                        _writableToAddress.Remove(endpoint);
                    }
                    else if (policy is RGEndpointManager.RGGated)
                    {
                        // already gated
                    }
                }
        ...
        {
            _addressToWritable[address] = new RGEndpointManager.RGQuarantined(uid, timeOfRelease);
        ...
        public void Prune()
        {
            _addressToWritable = _addressToWritable.Where(x =>
            {
                switch (x.Value)
                {
                    case RGEndpointManager.RGGated g when g.TimeOfRelease.HasTimeLeft:
                    case RGEndpointManager.RGQuarantined q when q.Deadline.HasTimeLeft:
                    case RGEndpointManager.RGPass p:
                        return true;
                    default:
                        return false;
                }
